    ...
    if ( sDate != null && sDate.length > 0 ) {
        ...
        if (this.separator[0] && this.separator[1]) {
            ...
            for ( var i=0; i<this.order.length; i++ ) {
                switch ( this.order[i] ) {
                case 0:
                    aYear = temp.substring( 0, this.sizes[0] );
                    temp = temp.substring( this.sizes[0] );
                    break;
                case 1:
                    aMonth = temp.substring( 0, this.sizes[1] );
                    temp = temp.substring( this.sizes[1] );
                    break;
                case 2:
                    aDate = temp.substring( 0, this.sizes[2] );
                    temp = temp.substring( this.sizes[2] );
                    break;
                }
            }
        }

        // check for single digit year (e.g. 1/1/1)
        // Fix for WPF Bug # BowSt00028191
        if(aYear.length == 1)
            aYear = "0" + aYear;
...
function WDateChooser_setVisible( visible )
{
    ...
    if ( visible  && !this.layer.isVisible() )
    {
        ...
        // set layer position
        this.layer.setPosition( new Position( posX, posY, 100 ) );
...
function WDateChooser_buildCalendarDiv( year, month, date ) {
    ...
    if ( div == null ) {
        ...
        // In IE 7, the table will default to zero width.
        if (WClient.isBrowserInternetExplorer() && WClient.isBrowserVersion7Up())
        {      
        ...
        if ( this.navMode == 1 ) {
            ...
            yopt.appendChild( document.createTextNode( "0000" ) );
            sel.onchange = WDateChooser_updateCalendarViewHandler;
...
function WDateChooser_selectDate( date, dismiss, aDoc ) {
    if ( date != null ) {
        ...
        if ( dismiss ) {
            ...
            if(dateInput.fireEvent) dateInput.fireEvent("onchange");
            else {
              if(dateInput.onchange) dateInput.onchange();
           }
...
function WDateChooser_prevMonthHandler( event ) {
    ...
    if ( dc != null ) {
        ...
        WDateChooser_ensureChooserVisibility( dc );    
    }
}
...
function WDateChooser_ensureChooserVisibility( dc ) {
    if (dc && WClient.isBrowserInternetExplorer() && WClient.isBrowserVersion7Up() ) {
        // In IE 7, view will be hidden after update: force it to be visible.
        dc.layer.visible = false;
        dc.layer.setVisible(true);
    }
}
