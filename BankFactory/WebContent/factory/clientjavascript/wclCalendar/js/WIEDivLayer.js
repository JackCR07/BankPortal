...
function WIEDivLayer_render()
{
    ...
    if ( !container )
    {
        ...
        with ( iframeElement )
        {
            ...
            // Can use "about:blank" except in pre-7 IEs running under https.
            var blankIsOK = WClient.isBrowserVersion7Up() || location.protocol != "https:";
            var blank = blankIsOK ? "about:blank" : "javascript:0";               
            src = ( this.secureURL != null ? this.secureURL : blank ); // @01A1
...
{
    this.position = ( position ? position : new Position( 0, 0, 100 ) );
    ...
    if ( this.visible != visible )
    {
        ...
        if ( container && transparentElement )
        {
            if ( this.visible )
            {
                ...
                // Hack: in IE 7 with strict html dtd, width is zero here.
                if (WClient.isBrowserInternetExplorer() && WClient.isBrowserVersion7Up() && container.style.width == "0px")
