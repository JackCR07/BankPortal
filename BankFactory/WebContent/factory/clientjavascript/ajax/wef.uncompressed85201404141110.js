...
{
var wpf_io85201404141110 = {
  bind: function(opts) {
	wpf_io85201404141110 =
    ...
    if (xh.xmlhttp.readyState == 4){
      if (xh.xmlhttp.responseText){      
        ...
        if (xh.xmlhttp.responseXML && /application.xml/.test(xh.xmlhttp.getResponseHeader("Content-Type"))){
          var feed = wpf_io85201404141110.findElement(xh.xmlhttp.responseXML, "feed");
          if (feed){
            var entry = wpf_io85201404141110.findElement(feed, "entry");
            if (entry){
              var c = wpf_io85201404141110.findElement(entry, "content");
              if (c && c.firstChild)
...
serializeElement: function(el){
  ...
  return wpf_io85201404141110.bind(opts);  
}
}
}
if (typeof wpf_ppr85201404141110 == "undefined")
{
  var wpf_ppr85201404141110 = {
    ...
    execute: function(url, form, idlist, hdlr, handlerOpts){
      if (!hdlr)
        hdlr = wpf_ppr85201404141110.explicitHandler;
      if (hdlr.prepare)
      ...
      var options ={
        ...
        load: function(type, data, evt){
          ...
          if (!hdlr.update(data, idlist, handlerOpts))
            wpf_ppr85201404141110.checkSessionTimeout(data);
    ...
    endLoad: function(){},
    bind: function(opts){
      wpf_io85201404141110.bind(opts);
    },
    ...
    removeControls: function(element){
      if (element.nodeType == 1 ){
        ...
        for(var i=0; i < children.length; i++){
          wpf_ppr85201404141110.removeControls(children[i]);
        }
      }
    },
    smartRefreshHandler:{
      ...
      update: function(data, idlist){
        ...
        try{
          ...
          ok = wpf_ppr85201404141110.explicitHandler.update(data, idlist, {execScripts:!dojoAvailable, useDojo:dojoAvailable});
          if (ok){
      ...
      update: function(data,idlist){
        if (data){
          for (var i = 0; i < wpf_ppr85201404141110.wrapperPatterns.length; i++){
            var regex = wpf_ppr85201404141110.wrapperPatterns[i];
    ...
    directHandler:{
      update: function(data, idlist){
        ...
        wpf_ppr85201404141110.buildWidgets(idlist);
        return true;
      }
    },
    explicitHandler:{
      update: function(data, idlist, handlerOpts){
        ...
        if (idlist && idlist[0] == document.body.id) {
          ok = wpf_ppr85201404141110.explicitHandler.injectContent(document.body, data, handlerOpts && handlerOpts.useDojo);
        }else{
          tmp.innerHTML = data;
          var nchildren = tmp.childNodes.length;
          for (var n = 0; n < nchildren; n++){
            var node = tmp.childNodes[n];
            wpf_ppr85201404141110.updateCSS(node, idlist);
            ok = wpf_ppr85201404141110.explicitHandler.updateContent(node, idlist, handlerOpts && handlerOpts.useDojo) || ok;
          }
        }
        if (handlerOpts && handlerOpts.execScripts)
          wpf_ppr85201404141110.executeScripts(document, idlist);
      ...
      updateContent: function(node, idlist, useDojo){
        ...
        for (var c = node.firstChild; c != null; c = c.nextSibling){
          ok = wpf_ppr85201404141110.explicitHandler.updateContent(c, idlist, useDojo) || ok;
        }
        return ok;
      },
      ...
      try{
        if (idlist){
          for (var i = 0; i < idlist.length; i++){
            ...
            wpf_ppr85201404141110.updateCSS(subel);
          }
          return;
        }
        if (!el)
    ...
    executeScripts: function(el, idlist){
      if (idlist){
        for (var i = 0; i < idlist.length; i++){
          ...
          if (el.id == idlist[i])
            ...
            subel = el.getElementById(idlist[i]);
          wpf_ppr85201404141110.executeScripts(subel);
        }
        return;
      }
      if (!el)
      ...
      for (var i = 0; i < scripts.length; i++){
        ...
        var src = script.src;
        if (src){
          wpf_ppr85201404141110.addScriptReference({src: src, deferred: deferred});
        }else{
    ...
    evaluateScriptText: function(scptTxt){
      ...
      window._wpf_ppr85201404141110_global_eval (scptTxt);
    },
